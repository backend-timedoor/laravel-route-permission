...
namespace Timedoor\RoutePermission\Traits;
...
use Timedoor\RoutePermission\Exceptions\RoutePermissionDoesNotExist;
...
trait HasPermissions
{
    ...
    /**
     * Determine if the model may perform the given permission for route.
     *
     * @param string $uri
     ...
     *
     * @return bool
     * @throws RoutePermissionDoesNotExist
     */
    public function hasPermissionForRoute($uri, $method = true, $guardName = null): bool
    {   
        ...
        if ($throw) {
            throw RoutePermissionDoesNotExist::related($routePermission->getUri(), $routePermission->getMethod(), $guardName);
        }
    ...
    /**
     * Determine if the model may perform the exact given permission for route.
     *
     * @param string $uri
     * @param string|bool $method
     * @param string|null $guardName
     ...
     * @throws RoutePermissionDoesNotExist
     */
    public function hasPermissionForExactRoute($uri, $method = true, $guardName = null): bool
    {     
        $permission = (new RoutePermission($uri, $method))->getPermission();

        try {
            return $this->hasPermissionForRoute($uri, $method, $guardName);
        } catch (PermissionDoesNotExist $e) {
            throw new RoutePermissionDoesNotExist();
        }
    }
    ...
    /**
     * An alias to hasPermissionForRoute(), but avoids throwing an exception.
     *
     * @param string $uri
     * @param string|bool $method
     * @param string|null $guardName
     ...
     * @return bool
     */
    public function checkPermissionForRoute($uri, $method, $guardName = null): bool
    {
        try {
            return $this->hasPermissionForRoute($uri, $method, $guardName);
        } catch (RoutePermissionDoesNotExist $e) {
            return false;
        }
    }
    ...
    /**
     * Determine if the model has any of the given permissions for route.
     *
     * @param array $routePermissions
     *
     * @return bool
     */
    public function hasAnyPermissionForRoute($routePermissions): bool
    {
        $permissions = [];

        foreach ($routePermissions as $route) {
            $uri = $route['uri'];
            $method = $route['method'] ?? true;

            $permissions = array_merge(
                $permissions,
                (new RoutePermission($uri, $method))->getPermissions()
            );
        }
    ...
    /**
     * Determine if the model has any of the given permissions for exact route.
     *
     * @param array $routePermissions
     *
     * @return bool
     */
    public function hasAnyPermissionForExactRoute($routePermissions): bool
    {
        $permissions = [];
        ...
        foreach ($routePermissions as $route) {
            ...
            $permissions[] = (new RoutePermission($uri, $method))->getPermission();
        }
     ...
     * @return $this
     */
    public function givePermissionForRoute($uri, $method = true)
    {
        $routePermission = new RoutePermission($uri, $method);

        $permission = getRoutePermissionName($routePermission->getUri(), $routePermission->getMethod());
     ...
     * @return $this
     */
    public function revokePermissionForRoute($uri, $method = true)
    {
        $routePermission = new RoutePermission($uri, $method);
